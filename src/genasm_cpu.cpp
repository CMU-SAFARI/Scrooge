...
using namespace std;
namespace genasm_cpu {
    bool enabled_algorithm_log = true;
    ...
    void genasm_kernel(vector<str> &refs, vector<str> &reads, vector<char*> &cigars, vector<long long> &edit_distances, int threads=1){
    ...
    std::vector<Alignment_t> align_all(Genome_t &reference, std::vector<Read_t> &reads, int threads, long long* core_algorithm_ns){
        ...
        long long ns = measure_ns([&](){
            genasm_kernel(refs, rds, cigars, edit_distances, threads);
        });
